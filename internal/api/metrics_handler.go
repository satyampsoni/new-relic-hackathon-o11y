paimport (
	...
	"github.com/satyampsoni/new-relic-hackathon-o11y/internal/alerts"
	"github.com/satyampsoni/new-relic-o11y/new-relic-hackathon-o11y/internal/config"
	"github.com/satyampsoni/new-relic-o11y/new-relic-hackathon-o11y/internal/staleness"
	"github.com/sirupsen/logrus"
)
...
// SystemStats returns system performance metrics
...
// RegisterRoutes registers all metrics API routes
func (h *MetricsHandler) RegisterRoutes(router *mux.Router) {
	// Metrics endpoints for New Relic Flex
	router.HandleFunc("/api/staleness/status", h.StalenessStatus).Methods("GET")
	router.HandleFunc("/api/health", h.HealthStatus).Methods("GET")
	router.HandleFunc("/api/alerts/summary", h.AlertsSummary).Methods("GET")
	router.HandleFunc("/api/system/stats", h.SystemStatsHandler).Methods("GET")
	router.HandleFunc("/api/performance/summary", h.PerformanceSummaryHandler).Methods("GET")
	
	// Additional utility endpoints
	router.HandleFunc("/api/config/validate", h.validateConfig).Methods("GET")
	router.HandleFunc("/api/test/generate-data", h.generateTestData).Methods("POST")
}
